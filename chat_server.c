               ...
               printf("value is inside %d %d\n", i, master_socket);
                if(i == master_socket){
                    ...
                    client_socket , inet_ntoa(cli_addr.sin_addr) ,
                     ...
                     recieve_id = recv(client_socket, client_Message, BUFF_SIZE, 0);
                     if (recieve_id < 0) {
                       perror("error recieving user name");
                     }

                     if (check_first_User == 0) {
                       memset(message, 0, BUFF_SIZE);
                       snprintf(message, BUFF_SIZE, "You are the first user to join the chat\0");
                     }

                     else {
                           memset(message, 0, BUFF_SIZE);

                           sprintf(message, "%s has just connected", client_Message);
