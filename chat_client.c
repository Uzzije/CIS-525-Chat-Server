...
int main(int argc, char **argv){
    ...
    while(chat == 1){
        ...
        if(strcmp(output_message, "1\n") == 0){
        ...
        else if(strcmp(output_message, "2\n") == 0){
            printf("hit in here\n");
            memset((char *) &serv_addr, 0, sizeof(serv_addr));
            serv_addr.sin_family  = AF_INET;
            serv_addr.sin_port = htons(8909);
            memcpy((void *)&serv_addr.sin_addr, hp->h_addr_list[0], hp->h_length);
            ...
            if(connect_id < 0){
                ...
                perror("client: can't connect to server port number");
        ...
        else if(strcmp(output_message, "3\n") == 0){
            memset((char *) &serv_addr, 0, sizeof(serv_addr));
            serv_addr.sin_family  = AF_INET;
         		...
         		if(FD_ISSET(i, &testfds)){
         			if(i==sockfd){
        				...
        				if(rec_id < 0){
         				...
         				if(strcmp(output_message, "new room\n") == 0){
                            memset(server_message, 0, MAX);
                            rec_id = recv(sockfd, server_message, MAX, 0);
                            if(rec_id < 0){
                                perror("didn't recieve message");
